...
class ConnectWorkersPage(BaseWebPage):
    ...
    def click_and_verify_status_count_breakdown_for_item(self, params):
        ...
        header = self.find_element_or_fail(table, By.XPATH, header_xpath, f"Deliver column{column_name}")
    ...
    def verify_green_bar_status_present(self, worker_name):
        ...
        cell_element = self.find_element_or_fail(row, By.XPATH, cell_xpath, f"Learn table column {column_index}")
        positive_condition = ('positive' in cell_element.get_attribute("class")) and ('black' not in cell_element.get_attribute("class"))
