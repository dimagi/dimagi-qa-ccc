...
from (WorkAreaTileView, tile_fields include id/slug/status/assignee) -
`fetch_all_work_areas` decodes it with the `mapbox-vector-tile` package. This
is a real product endpoint, not a guess: `get_flw_work_areas_for_assignment`
(used by `fetch_a_real_work_area` below) only returns work areas *currently
assigned to* a given assignee, which is empty for e.g. REQUEST_FOR_INACCESSIBLE
work areas that were seeded directly rather than routed through assignment.
"""
...
def _deg2tile(lat_deg, lon_deg, zoom):
    """Standard slippy-map tile math (lon/lat -> tile x/y at a zoom level)."""
...
class MicroplanningPage(BasePage):
    ...
    REVIEW_INACCESSIBLE_BUTTON = locators.get("connect_microplanning_page", "review_inaccessible_button")
    REVIEW_INACCESSIBILITY_FRAGMENT = locators.get("connect_microplanning_page", "review_inaccessibility_fragment")
    REVIEW_MODAL_BACKDROP = locators.get("connect_microplanning_page", "review_inaccessibility_modal_backdrop")
    REVIEW_VISIT_DATE_VALUE = locators.get("connect_microplanning_page", "review_visit_date_value")
    REVIEW_REASON_VALUE = locators.get("connect_microplanning_page", "review_reason_value")
    REVIEW_PHOTO_EVIDENCE_SECTION = locators.get("connect_microplanning_page", "review_photo_evidence_section")
    REVIEW_RETURN_TO_MAP_BUTTON = locators.get("connect_microplanning_page", "review_return_to_map_button")
    REVIEW_DENY_BUTTON = locators.get("connect_microplanning_page", "review_deny_button")
    REVIEW_APPROVE_BUTTON = locators.get("connect_microplanning_page", "review_approve_button")
    ...
    def fetch_all_work_areas(self, host, org_slug, opp_id):
        """Every WorkArea in the opportunity (id/slug/status/assignee/...), decoded
        from the same Mapbox Vector Tile the map canvas itself renders - see the
        module docstring for why this exists instead of an assignee/group-scoped
        endpoint.

        Finding a covering tile without a bbox endpoint: `workareas_group_geojson`
        documents a `workarea_bounds` key (full-opportunity extent) in the local
        commcare-connect checkout, but the deployed staging build's response
        omits it entirely (confirmed live 2026-09-23 - another deployed-vs-repo
        divergence, same pattern flagged elsewhere in this codebase) - so this
        can't seed the search from a real bbox. Instead it downloads the CSV
        export for one real centroid, then samples a spread of zoom levels
        around that point and keeps whichever tile decoded the most features -
        good enough for a single opportunity's work areas, which cluster
        tightly in every opp built so far. A geographically sprawling
        opportunity would need real bbox stitching; not built, since nothing
        has needed it yet."""
        if not self.download_button_present():
            return []
        download = self.click_download()
        path = download.path()
        ...
        if not rows:
            return []
        lon_str, lat_str = rows[0]["Centroid"].split()
        lon, lat = float(lon_str), float(lat_str)

        best_work_areas = []
        for zoom in (6, 8, 10, 12, 14, 16):
            x, y = _deg2tile(lat, lon, zoom)
            url = f"{host}/a/{org_slug}/microplanning/{opp_id}/tiles/{zoom}/{x}/{y}/"
            resp = self.page.request.get(url)
            if not resp.ok or not resp.body():
                continue
            decoded = mapbox_vector_tile.decode(resp.body())
            features = decoded.get("workareas", {}).get("features", [])
            if len(features) > len(best_work_areas):
                best_work_areas = [f["properties"] for f in features]
        self._step(f"Fetched {len(best_work_areas)} work area(s) via vector tile")
        return best_work_areas
    ...
    # The trigger button is x-show="selectedFeature.status === 'REQUEST_FOR_INACCESSIBLE'"
    # in the same "Select Work Area" panel select_work_area_via_js populates, so a
    # REQUEST_FOR_INACCESSIBLE work area selected that way is enough to reach it -
    # no map click needed here either.
    #
    # Confirmed live 2026-09-23 on staging: the backend is fully deployed (GET
    # .../review_inaccessibility/<id>/ returns 200 with real request data,
    # Frontline Worker/Date of Visit/Reason) but the FRONTEND trigger isn't -
    # neither the "Review Inaccessible" button nor Alpine's
    # openReviewInaccessibilityModal() exist anywhere on the rendered page, even
    # with a REQUEST_FOR_INACCESSIBLE work area selected. Deployed-vs-repo
    # divergence, same pattern flagged elsewhere in this codebase, just the
    # opposite direction (repo ahead of deploy, not behind). review_button_ready()
    # lets callers skip cleanly instead of a 10s+ timeout hunting a button that
    # isn't there yet.

    def review_button_ready(self):
        return self.page.locator(self.REVIEW_INACCESSIBLE_BUTTON).count() > 0

    def open_review_inaccessibility_modal(self):
        self._step("Open 'Review Inaccessible' modal")
        self.click(self.REVIEW_INACCESSIBLE_BUTTON)
        self.page.locator(self.REVIEW_INACCESSIBILITY_FRAGMENT).locator(self.REVIEW_DENY_BUTTON).first.wait_for(
            state="visible", timeout=10000
        )

    def review_modal_fields_present(self):
        present = {
            "visit_date": self.page.locator(self.REVIEW_VISIT_DATE_VALUE).count() > 0,
            "reason": self.page.locator(self.REVIEW_REASON_VALUE).count() > 0,
            "photo_evidence": self.page.locator(self.REVIEW_PHOTO_EVIDENCE_SECTION).count() > 0,
            "return_to_map": self.page.locator(self.REVIEW_RETURN_TO_MAP_BUTTON).count() > 0,
            "deny": self.page.locator(self.REVIEW_DENY_BUTTON).count() > 0,
            "approve": self.page.locator(self.REVIEW_APPROVE_BUTTON).count() > 0,
        }
        self._step(f"Review-inaccessibility modal fields present: {present}")
        return present

    def review_visit_date(self):
        return self.page.locator(self.REVIEW_VISIT_DATE_VALUE).first.inner_text().strip()

    def review_reason(self):
        return self.page.locator(self.REVIEW_REASON_VALUE).first.inner_text().strip()

    def deny_inaccessibility_request(self):
        """Submits the review form with action=deny (server maps this to
        WorkAreaStatus.NOT_VISITED and sends the worker a push notification).
        The 204 response's HX-Trigger (inaccessibilityReviewed) closes the modal
        and updates selectedFeature.status client-side - wait on the backdrop
        (x-show wrapper), not the fragment div, which stays in the DOM."""
        self._step("Deny inaccessibility request")
        self.click(self.REVIEW_DENY_BUTTON)
        self.page.locator(self.REVIEW_MODAL_BACKDROP).first.wait_for(state="hidden", timeout=15000)

    def approve_inaccessibility_request(self):
        """Submits the review form with action=approve (server maps this to
        WorkAreaStatus.INACCESSIBLE, no notification - approve is the worker's
        own request being confirmed)."""
        self._step("Approve inaccessibility request")
        self.click(self.REVIEW_APPROVE_BUTTON)
        self.page.locator(self.REVIEW_MODAL_BACKDROP).first.wait_for(state="hidden", timeout=15000)

    def selected_feature_status(self):
