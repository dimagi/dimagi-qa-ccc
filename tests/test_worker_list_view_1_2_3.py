...
def test_worker_list_view_1_2_3_verify_connect_workers_details_of_opportunity_in_connect(web_driver, test_data, config):
    worker_list_view_1_data = test_data.get("WORKER_LIST_VIEW_1")
    worker_list_view_2_data = test_data.get("WORKER_LIST_VIEW_2")
    worker_list_view_3_data = test_data.get("WORKER_LIST_VIEW_3")
    ...
    with allure.step("Navigate and verify Connect Workers details in Opportunity"):
        connect_opp_page.click_link_by_text(worker_list_view_1_data["opportunity_name"])
        opp_dashboard_page.click_dashboard_card_in_opportunity(worker_list_view_1_data["card_title"], worker_list_view_1_data["card_subtitle"])
        opp_dashboard_page.verify_text_in_url(worker_list_view_1_data["url_text"])
        connect_workers_page.verify_table_headers_present(worker_list_view_1_data["headers_list"])

    # Worker List View_2
    with allure.step("Navigate and verify Connect Workers Learn tab details in Opportunity"):
        connect_workers_page.click_tab_by_name(worker_list_view_2_data["tab_name"])
        connect_workers_page.verify_table_headers_present(worker_list_view_2_data["headers_list"])

    # Worker List View_3
    with allure.step("Navigate and verify Connect Workers Deliver tab details in Opportunity"):
        connect_workers_page.click_tab_by_name(worker_list_view_3_data["tab_name"])
        connect_workers_page.verify_table_headers_present(worker_list_view_3_data["headers_list"])